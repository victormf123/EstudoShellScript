...
# Data: 13/06/2019                                                  #
#                                                                   #
# Descrição: Script com diversas funções que fazem tratamentos com  #
# datas                                                             #
# Uso: ./FuncoesDiversas.sh  --help                                 #
#      ./FuncoesDiversas.sh OPÇÄO DATA                              #
#                                                                   #
# Opções:                                                           # 
# -f = Retorna 0 para BR, 1 para US, 2 quando impossível determinar #
#      e 3 para formatode data inválida                             #
...
RetornaFormato () {
    ...
    # e 2 quando é impossivel determinar. quando dia = mês, considera-se formato BR.
    ...
    if [ $VAL1 -le 12 -a $VAL1 -eq $VAL2 ] # Dia = Mes = Formato BR
        ...
        then
            return 1
        elif [ $VAL1 -le 12 -a $VAL2 -gt 12 ] # Formato Indefinido, mas válido
        then 
            return 2
        else
            return 3
...
InverteDiaMes () {
    #Função de inverte os valores do dia com mês e retorna o valor
    RetornaFormato $1
    local FORMATO=$?
    if [ $FORMATO -ne 3 ]
    then    
...
IncluiBarra () {
    #Função que inclui as barras da data
    RetornaFormato $1
    local FORMATO=$?
    if [ $FORMATO -ne 3 ]
    then    
...
case $1 in
        ...
        "--help")
            ...
            echo "-i = Inclui Barras de Data. Exemplo: de 12081981 para 13/08/1981"
            echo "-b = Inclui Barras de Data. Exemplo de 13081981 para 13 de Agosto de 1981 "
            ;;
        *)
